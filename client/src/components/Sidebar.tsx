...
export default function Sidebar({ onOpenSettings, onOpenNewChat, onOpenNotifications }: Props) {
    ...
    <aside className="glass relative flex w-[340px] shrink-0 flex-col rounded-[28px] m-3 mr-1 overflow-hidden">
      ...
      <div className="flex items-center gap-3 border-b border-white/10 px-4 py-4">
        ...
        <div className="ml-auto flex items-center gap-1">
          <IconButton onClick={onOpenNotifications} title="Notifications">
      ...
      <div className="px-3 pt-3">
        <div className="glass-soft rounded-2xl p-1 flex relative">
          {(['chats', 'feed'] as const).map((v) => (
            <button
              key={v}
              type="button"
              onClick={() => setView(v)}
              className={clsx(
                'flex-1 py-1.5 rounded-xl text-xs uppercase tracking-[0.2em] transition relative z-10',
                view === v ? 'text-white' : 'text-[var(--text-muted)]',
              )}
            >
              {view === v && (
                <motion.span
